...
"""Low-overhead, process-safe trajectory spans for AI-search experiments."""
...
def _sampled(trace_id: str, sample_rate: float) -> bool:
    ...
    value = int.from_bytes(digest, byteorder="big") / float(2**64)
...
class TraceSpan:
    """Write one completed JSONL span when the trajectory is sampled."""
    ...
    def set_attributes(self, **attributes: Any) -> None:
        """Attach bounded metadata that becomes visible when the span closes."""
...
def trace_span(
    *,
    ...
    """Return a span context manager; tracing is disabled when no path is set."""
    return TraceSpan(
        trace_id=trace_id,
        component=component,
        operation=operation,
